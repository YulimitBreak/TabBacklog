...
abstract class ListRetrieveResolver<T, Query : RetrieveQuery<T>>(val source: List<T>) : RetrieveResolver<T, Query>() {
    ...
    final override suspend fun fetchFlow(query: Query?, hasReorderingActions: Boolean): Flow<T> =
        (query?.let { applyQueryToList(source, query) } ?: source).asFlow()

    final override suspend fun applyQueryToFlow(flow: Flow<T>, query: Query): Flow<T> =
